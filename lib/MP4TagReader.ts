...
export default class MP4TagReader extends MediaTagReader {
    ...
    _parseData(data: MediaFileReader, tagsToRead: string[]): TagType {
        ...
        tagsToRead = this._expandShortcutTags(tagsToRead) as string[];
        this._readAtom(tags, data, 0, data.getSize(), tagsToRead);

        // create shortcuts for most common data.
        for (var name in SHORTCUTS)
            if (SHORTCUTS.hasOwnProperty(name)) {
                var tag: any = tags[(SHORTCUTS as any)[name]];
                if (tag) {
                    if (name === "track") {
    ...
    _readAtom(
        ...
        offset: number,
        length: number,
        tagsToRead?: string[] | undefined | null,
        parentAtomFullName?: string,
        indent?: string
    ) {
    ...
    _readMetadataAtom(data: MediaFileReader, offset: number): TagFrame {
        ...
        return {
            id: atomName,
            size: atomSize,
            description: (ATOM_DESCRIPTIONS as any)[atomName] || "Unknown",
            data: atomData,
        };
    }
