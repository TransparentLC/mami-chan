...
type T_FrameDescriptorKey = keyof typeof _FRAME_DESCRIPTIONS;
type TTextFrameDescriptorKey = keyof typeof TEXT_FRAME_DESCRIPTIONS;
type TUrlFrameDescriptorKey = keyof typeof URL_FRAME_DESCRIPTIONS;
type TFrameDescriptorKey =
    | T_FrameDescriptorKey
    | TTextFrameDescriptorKey
    | TUrlFrameDescriptorKey;

// type TFrameReadFunction = (offset:number,length:number,data:MediaFileReader,flags?:Object, id3header?: TagHeader)=>any;
type TFrameReadFunction = FrameReaderSignature;

export default class ID3v2FrameReader {
    static getFrameReaderFunction(
        frameId: TFrameDescriptorKey
    ): FrameReaderSignature | null {
    ...
    static readFrames(
        ...
        var frames: Partial<Record<TFrameDescriptorKey, any>> = {};
        ...
        while (
            ...
            var frameId = header.id as TFrameDescriptorKey | string;
            ...
            var readFrameFunc = ID3v2FrameReader.getFrameReaderFunction(
                frameId as TFrameDescriptorKey
            );
            ...
            let sframeId = frameId as TFrameDescriptorKey;
            if (frameId in frames) {
                if (frames[frameId as TFrameDescriptorKey].id) {
    ...
    static _readFrameHeader(
        ...
        let frameId: TFrameDescriptorKey | "" = "";
        let frameSize = 0;

        switch (major) {
            case 2:
                frameId = data.getStringAt(offset, 3) as TFrameDescriptorKey;
                frameSize = data.getInteger24At(offset + 3, true);
                break;

            case 3:
                frameId = data.getStringAt(offset, 4) as TFrameDescriptorKey;
    ...
    static _getFrameDescription(frameId: string): string {
        if (frameId in FRAME_DESCRIPTIONS) {
            return FRAME_DESCRIPTIONS[frameId as TFrameDescriptorKey];
        } else {
            return "Unknown";
        }
    }
...
const frameReaderFunctions: Partial<
    Record<TFrameDescriptorKey, TFrameReadFunction>
> = {
...
const textFrameReaderFunctions: Partial<
    Record<TTextFrameDescriptorKey, TFrameReadFunction>
> = {
    TCO: readGenreFrame,
    TCON: readGenreFrame,
...
const urlFrameReaderFunctions: Partial<
    Record<TUrlFrameDescriptorKey, TFrameReadFunction>
> = {
    WXXX: readUserDefinedUrlFrame,
};

const allFrameReaderFunctions: Partial<
    Record<TFrameDescriptorKey, TFrameReadFunction>
> = {
...
function readPictureFrame(
    ...
    length: number,
    data: MediaFileReader,
    flags: Object | null | undefined,
    id3header?: TagHeader
): any {
    let start = offset;
    let charset = getTextEncoding(data.getByteAt(offset));
    let format: string | InternalDecodedString;
...
// CHAP
function readChapterFrame(
    this: typeof ID3v2FrameReader,
    offset: number,
    length: number,
    data: MediaFileReader,
    flags: Object | null | undefined,
    ...
    let result: any = {};
    let id = StringUtils.readNullTerminatedString(
...
// TXXX
function readUserDefinedTextFrame(
    offset: number,
    length: number,
    data: MediaFileReader,
    flags: Object | null | undefined,
...
// WXXX
function readUserDefinedUrlFrame(
    offset: number,
    length: number,
    data: MediaFileReader,
    flags: Object | null | undefined,
...
// UFID
function readFileIdentifierFrame(
    offset: number,
    length: number,
    data: MediaFileReader,
    flags: Object | null | undefined,
...
function getTextEncoding(bite: number): CharsetType {
    var charset: CharsetType | undefined;
