...
export default class ID3v2TagReader extends MediaTagReader {
    ...
    _parseData(data: MediaFileReader, tags?: Array<string>): TagType {
        ...
        var id3 = {
            ...
            size: size,
            tags: {} as Partial<Record<keyof typeof SHORTCUTS, any>>,
        };
        ...
        for (var name in SHORTCUTS)
            if (SHORTCUTS.hasOwnProperty(name)) {
                var frameData = this._getFrameData(
                    frames,
                    SHORTCUTS[name as keyof typeof SHORTCUTS]
                );
                if (frameData) {
                    id3.tags[name as keyof typeof SHORTCUTS] = frameData;
                }
            }

        for (var frame in frames)
            if (frames.hasOwnProperty(frame)) {
                // i cant be bothered to fix this
                (id3.tags as any)[frame] = frames[frame];
            }
